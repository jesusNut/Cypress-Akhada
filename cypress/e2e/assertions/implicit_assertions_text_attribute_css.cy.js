...
//* Asserting CSS.
//* Asserting text using cy.contains()
...
describe('Fetching and asserting CSS', () => {
    it('Fetching css - invoke() method', () => {

        cy.visit('http://www.webdriveruniversity.com/Actions/index.html');
        cy.get('#double-click').invoke('css', 'background-color').then((fetchedColor) => {
            cy.log(fetchedColor);
        })
    });

    it('Fetching css - jquery css() method directly in then() block', () => {
...
describe('Asserting text using cypress own contains() method ', () => {
    it('demo1', () => {

        cy.visit('http://uitestingplayground.com/clientdelay');
        cy.get('#ajaxButton').click();
        cy.get('#content>p',{timeout:20000}).contains('Data calculated');

    });
    it('demo2', () => {

        cy.visit('http://omayo.blogspot.com/');
        cy.get('#HTML26>div:nth-of-type(1)>ul>li').first()
        .contains('faltu text',{timeout:20000});
        //contains will assert if the element i.e. first li
        //contains and retry for 20 seconds.
        //FYI, above will fail. Check timing in error message.
